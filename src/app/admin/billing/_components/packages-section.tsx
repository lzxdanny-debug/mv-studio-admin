...
function PackageRow({
  ...
  const cellInput =
    'w-full px-2 py-1 text-sm border border-transparent hover:border-slate-200 focus:border-blue-400 rounded focus:outline-none bg-transparent';
          ...
          {isNew ? (
            ...
            <>
              <button
                ...
                className="p-1.5 rounded text-blue-600 hover:bg-blue-50 disabled:opacity-50 disabled:cursor-not-allowed"
                title={dirty ? '保存' : '已保存'}
              >
                {saving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />}
              </button>
