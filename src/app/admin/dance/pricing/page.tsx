...
function rowKey(r: DancePriceRow) {
  return `${r.step}|${r.resolution}`;
}

export default function AdminDancePricingPage() {
  const qc = useQueryClient();
  const canEdit = useAdminAuthStore((s) => s.hasPermission('dance.pricing.edit'));
  ...
  const { data, isLoading, isError, error } = useQuery<DancePricingView>({
    queryKey: ['admin', 'dance', 'pricing'],
    queryFn: () => apiClient.get('/admin/dance/pricing') as any,
  });
  ...
  const save = useMutation({
    mutationFn: () => {
      ...
      return apiClient.patch('/admin/dance/pricing', { items }) as any;
    },
    onSuccess: () => {
      setMsg({ ok: true, text: '舞蹈步骤价格已保存。' });
      qc.invalidateQueries({ queryKey: ['admin', 'dance', 'pricing'] });
    },
      ...
      <div className="space-y-5 p-6">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            dimension=dance 的步骤价格，与 MV / Karaoke 共用 step_prices 表。片段视频按项目实际片段数逐个计费，
            总价随音乐时长变化，因此这里只维护单价。未配置的步骤会阻止生成，不会使用代码默认价格。
          </p>
        </div>
                      ...
                      <td className="px-4 py-3 align-top">
                        ...
                        {openHelp === key && (
                          <p className="mt-1.5 max-w-md text-xs leading-relaxed text-slate-500">
                            {r.description}
                            {r.nonBillableReason ? `（${r.nonBillableReason}）` : ''}
                          </p>
                        )}
                      </td>
        ...
        {!canEdit && (
          <p className="text-xs text-amber-600">
            当前账号仅有 dance.pricing.view，无法保存（需 dance.pricing.edit）。
          </p>
        )}
