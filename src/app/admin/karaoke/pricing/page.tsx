...
function rowKey(r: KaraokePriceRow) {
      ...
      <div className="space-y-5 p-6">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            dimension=karaoke 的步骤价格，与 MV / Dance 共用 step_prices 表。口型视频按秒计费，
            总价随音乐时长变化，因此这里只维护单价。
          </p>
        </div>

        {rows.some((row) => !row.configured && row.billable) && (
          <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            还有 {rows.filter((row) => !row.configured && row.billable).length} 项价格未配置。
            请填写价格并启用。
          </div>
        )}
