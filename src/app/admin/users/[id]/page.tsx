...
export default function AdminUserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const adjustCreditsMutation = useMutation({
    mutationFn: (body: { amount: number; reason?: string }) =>
      apiClient.post(`/admin/users/${id}/credits`, body) as Promise<{
        adjusted: number;
        balance: number;
      }>,
    onSuccess: (res) => {
      ...
      setAdjustReason('');
      refetch();
      qc.invalidateQueries({ queryKey: ['admin', 'user', id, 'transactions'] });
      const signed =
        res.adjusted > 0 ? `+${res.adjusted.toLocaleString()}` : res.adjusted.toLocaleString();
      void alert({
        title: '积分已调整',
        description: `已调整 ${signed} 积分，当前余额 ${res.balance.toLocaleString()}`,
        variant: 'success',
      });
    },
    onError: (err: unknown) => {
  ...
  const parsedAdjustAmount = Math.floor(Number(adjustAmount));
  const adjustAmountValid =
    Number.isFinite(parsedAdjustAmount) && parsedAdjustAmount !== 0;
  const currentBalance = data?.credit.balance ?? 0;
  const projectedBalance = adjustAmountValid
    ? currentBalance + parsedAdjustAmount
    : null;
  const deductExceedsBalance =
    adjustAmountValid &&
    parsedAdjustAmount < 0 &&
    currentBalance + parsedAdjustAmount < 0;

  const submitAdjustCredits = () => {
    if (!adjustAmountValid || deductExceedsBalance || adjustCreditsMutation.isPending) return;
    ...
    <div className="flex-1 overflow-y-auto bg-slate-100">
      ...
      {adjustOpen && (
        <div
          ...
          onClick={() => !adjustCreditsMutation.isPending && setAdjustOpen(false)}
        >
          <div
          ...
          >
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-base font-semibold text-slate-900">调整积分</h2>
                <p className="mt-1 text-xs text-slate-500">
                  为 {user?.displayName || '该用户'} 增加或扣减积分。正数增加，负数扣减。
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="space-y-3">
              <div className="rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 text-xs text-slate-600">
                当前余额：<span className="font-semibold tabular-nums text-slate-900">{currentBalance.toLocaleString()}</span>
                {projectedBalance != null && !deductExceedsBalance && (
              ...
              <div>
                ...
                {deductExceedsBalance && (
                  <p className="mt-1 text-[10px] text-red-500">
                    扣减后余额不能为负，最多可扣减 {currentBalance.toLocaleString()} 积分
                  </p>
                )}
              </div>
              <div>
            ...
            <div className="mt-5 flex justify-end gap-2">
              <button
                ...
                className="rounded-lg px-3 py-1.5 text-xs text-slate-500 hover:bg-slate-100 disabled:opacity-50"
              >
                取消
              </button>
              <button
                type="button"
                onClick={submitAdjustCredits}
                disabled={
                  !adjustAmountValid ||
                  deductExceedsBalance ||
