...
interface MailLogEncryptionView {
  secretMasked: string;
  configured: boolean;
...
export function MailLogEncryptionSection() {
  const qc = useQueryClient();
  const [secret, setSecret] = useState('');
  const [msg, setMsg] = useState<{ ok: boolean; text: string } | null>(null);

  const { data, isLoading, isError, error } = useQuery<MailLogEncryptionView>({
    queryKey: ['admin', 'settings', 'mail-log-encryption'],
    queryFn: () =>
      apiClient.get(
        '/admin/settings/mail-log-encryption',
      ) as Promise<MailLogEncryptionView>,
  });
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      setMsg({
        ok: true,
        text: '已保存并热加载。更换密钥后旧邮件日志验证码可能无法解密。',
      });
      setSecret('');
    ...
    >
      ...
      >
        <SettingsPanel
          ...
          summary="加密邮件验证码日志（mail_send_logs）；冷启动可自动生成"
          footer={
            <SettingsSaveBar
              msg={msg}
