    ...
    {query.isLoading ? <div className="flex justify-center p-12"><Loader2 className="h-6 w-6 animate-spin text-violet-600" /></div> : <div className="grid gap-5 xl:grid-cols-2">{query.data?.map((row) => {
      ...
      return <section key={row.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="grid gap-4 p-5 sm:grid-cols-[150px_1fr]">
          ...
          <div className="grid gap-3"><label className="text-xs font-medium text-slate-600">英文名称<input disabled={!canEdit || save.isPending} value={String(value(row, 'nameEn') ?? '')} onChange={(event) => setDraft(row.id, 'nameEn', event.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm disabled:bg-slate-50" /></label><label className="text-xs font-medium text-slate-600">中文名称<input disabled={!canEdit || save.isPending} value={String(value(row, 'nameZh') ?? '')} onChange={(event) => setDraft(row.id, 'nameZh', event.target.value)} className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm disabled:bg-slate-50" /></label><label className="text-xs font-medium text-slate-600">封面图片 URL<input disabled={!canEdit || save.isPending} value={String(value(row, 'previewImageUrl') ?? '')} onChange={(event) => setDraft(row.id, 'previewImageUrl', event.target.value)} placeholder="https://..." className="mt-1 w-full rounded-lg border border-slate-200 px-3 py-2 text-sm disabled:bg-slate-50" /></label><div className="text-xs font-medium text-slate-600"><span>示例视频 URL</span><div className="mt-1 flex gap-2"><input disabled={!canEdit || save.isPending || generating} value={String(value(row, 'exampleVideoUrl') ?? '')} onChange={(event) => setDraft(row.id, 'exampleVideoUrl', event.target.value)} placeholder="https://.../style-preview.mp4" className="min-w-0 flex-1 rounded-lg border border-slate-200 px-3 py-2 text-sm disabled:bg-slate-50" />{canEdit && <button type="button" disabled={generatePreview.isPending || generating || !previewImageUrl.trim()} onClick={() => generatePreview.mutate({ id: row.id, body: drafts[row.id] ?? {} })} className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-sm font-medium text-violet-700 transition hover:bg-violet-100 disabled:cursor-not-allowed disabled:opacity-50">{submittingGeneration || generating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Video className="h-4 w-4" />}{generating ? '生成中' : exampleVideoUrl ? '重新生成' : '生成'}</button>}</div>{row.previewGenerationStatus === 'failed' && <p title={row.previewGenerationError || ''} className="mt-1.5 line-clamp-2 text-xs font-normal text-red-600">生成失败：{row.previewGenerationError || '请稍后重试'}</p>}{generating && <p className="mt-1.5 text-xs font-normal text-violet-600">正在按照模型优先级生成，失败会自动切换下一模型。</p>}</div></div>
        </div>
