...
export function ModelRoutingTab() {
  ...
  return <div className="w-full space-y-5">
    ...
    {canEdit && <section className="rounded-xl border border-slate-200 bg-white p-5"><h2 className="font-semibold">新增一级模型</h2><div className="mt-3 grid gap-3 md:grid-cols-3"><Input value={draft.code} onChange={(v) => setDraft({ ...draft, code: v })} placeholder="code，例如 veo" /><Input value={draft.nameEn} onChange={(v) => setDraft({ ...draft, nameEn: v })} placeholder="Name (English)" /><Input value={draft.descriptionEn} onChange={(v) => setDraft({ ...draft, descriptionEn: v })} placeholder="Description (English)" /></div><div className="mt-3 flex justify-end"><Action disabled={create.isPending || !draft.code || !draft.nameEn} onClick={() => create.mutate()}><Plus className="h-4 w-4" />保存并自动翻译</Action></div>{create.isError && <ErrorText error={create.error} />}</section>}
...
function RouteEditor({ model, catalog, canEdit }: { model: ProductModel; catalog: Catalog; canEdit: boolean }) {
  ...
  const move = (index: number, delta: number) => { const next = [...rows]; const target = index + delta; if (target < 0 || target >= next.length) return; [next[index], next[target]] = [next[target], next[index]]; setRows(next); };
  const add = () => { const capability = catalog.capabilities[0] ?? 'videoSingleRef'; const provider = catalog.providers.find((item) => (catalog.capabilityModels[capability]?.[item] ?? []).length > 0) ?? catalog.providers[0]; const exactModel = catalog.capabilityModels[capability]?.[provider]?.[0] ?? ''; setRows([...rows, { capability, provider, exactModel, priority: rows.length, timeoutSec: 900, maxAttempts: 1, enabled: true }]); };
  return <section className="rounded-xl border border-slate-200 bg-white p-5"><div className="flex items-center justify-between"><div><h2 className="font-semibold text-slate-900">{model.nameEn} <span className="font-mono text-xs text-slate-400">{model.code}</span></h2><p className="mt-1 text-xs text-slate-500">翻译：{model.translationStatus} · 用户端不会看到以下真实模型</p></div>{canEdit && <button onClick={add} className="text-sm text-violet-700">+ 添加候选</button>}</div><div className="mt-4 space-y-2">{rows.map((row, index) => <div key={`${index}-${row.capability}-${row.provider}-${row.exactModel}`} className="grid items-center gap-2 rounded-lg bg-slate-50 p-3 md:grid-cols-[36px_130px_130px_1fr_100px_70px_50px]"><span className="text-center font-semibold text-slate-400">{index + 1}</span><select disabled={!canEdit} value={row.capability} onChange={(e) => { const capability = e.target.value as VideoCapability; const provider = catalog.providers.find((item) => (catalog.capabilityModels[capability]?.[item] ?? []).length > 0) ?? row.provider; const next = [...rows]; next[index] = { ...row, capability, provider, exactModel: catalog.capabilityModels[capability]?.[provider]?.[0] ?? '' }; setRows(next); }} className="rounded border border-slate-200 bg-white px-2 py-2 text-sm">{catalog.capabilities.map((capability) => <option key={capability} value={capability}>{CAPABILITY_LABELS[capability]}</option>)}</select><select disabled={!canEdit} value={row.provider} onChange={(e) => { const provider = e.target.value as Provider; const next = [...rows]; next[index] = { ...row, provider, exactModel: catalog.capabilityModels[row.capability]?.[provider]?.[0] ?? '' }; setRows(next); }} className="rounded border border-slate-200 bg-white px-2 py-2 text-sm">{catalog.providers.map((p) => <option key={p} disabled={!(catalog.capabilityModels[row.capability]?.[p] ?? []).length}>{p}</option>)}</select><select disabled={!canEdit} value={row.exactModel} onChange={(e) => { const next = [...rows]; next[index] = { ...row, exactModel: e.target.value }; setRows(next); }} className="min-w-0 rounded border border-slate-200 bg-white px-2 py-2 text-sm">{(catalog.capabilityModels[row.capability]?.[row.provider] ?? []).map((m) => <option key={m}>{m}</option>)}</select><input disabled={!canEdit} type="number" value={row.timeoutSec} onChange={(e) => { const next = [...rows]; next[index] = { ...row, timeoutSec: Number(e.target.value) }; setRows(next); }} className="rounded border border-slate-200 px-2 py-2 text-sm" title="超时秒数" /><div className="flex gap-1"><button onClick={() => move(index, -1)}><ArrowUp className="h-4 w-4" /></button><button onClick={() => move(index, 1)}><ArrowDown className="h-4 w-4" /></button></div><button disabled={!canEdit} onClick={() => setRows(rows.filter((_, i) => i !== index))} className="text-red-600"><Trash2 className="h-4 w-4" /></button></div>)}</div>{canEdit && rows.length > 0 && <div className="mt-4 flex justify-end"><Action disabled={save.isPending} onClick={() => save.mutate()}><Save className="h-4 w-4" />保存候选链</Action></div>}{save.isError && <ErrorText error={save.error} />}</section>;
}
...
function CapacityFamilyEditor({ provider, family, exactModels, rows, canEdit }: { provider: Provider; family: string; exactModels: string[]; rows: Capacity[]; canEdit: boolean }) {
  ...
  return <div className="rounded-xl border border-slate-200 bg-slate-50/60 p-4">
    ...
    <div className="mt-4 grid gap-3 sm:grid-cols-2">
      <NumberInput label="全局并发" value={globalConcurrency} onChange={setGlobalConcurrency} />
      <NumberInput label="排队超时（秒）" value={queueTimeoutSec} onChange={setQueueTimeoutSec} />
    </div>
    {canEdit && <div className="mt-4 flex justify-end"><Action disabled={save.isPending || globalConcurrency < 1 || queueTimeoutSec < 1} onClick={() => save.mutate()}><Save className="h-4 w-4" />保存</Action></div>}
...
export function PricingTab() {
  ...
  const saveConfig = useMutation({ mutationFn: () => apiClient.put('/admin/aimv-generator/pricing/config', config), onSuccess: () => qc.invalidateQueries({ queryKey: ['aimv-pricing'] }) });
  if (query.isLoading || !config) return <Loading />;
  ...
  return <div className="w-full space-y-5">
    ...
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 px-5 py-4">
        ...
        <div className="flex items-center gap-4"><label className="flex items-center gap-2 text-sm font-medium text-slate-600"><span className={`h-2 w-2 rounded-full ${config.enabled ? 'bg-emerald-500' : 'bg-slate-300'}`} />{config.enabled ? '计费已启用' : '计费未启用'}<Switch checked={config.enabled} onChange={(enabled) => setConfig({ ...config, enabled })} disabled={!canEdit} label="启用 AI MV 计费" size="lg" /></label>{canEdit && <Action disabled={saveConfig.isPending} onClick={() => saveConfig.mutate()}><Save className="h-4 w-4" />保存策略</Action>}</div>
      </div>
      <div className="grid gap-4 p-5 xl:grid-cols-[minmax(420px,0.85fr)_minmax(640px,1.15fr)]">
        ...
        <div className="rounded-xl border border-violet-100 bg-violet-50/40 p-4"><div className="mb-4 flex items-start justify-between gap-3"><div><h3 className="text-sm font-semibold text-slate-800">统一价格倍率</h3><p className="mt-1 text-xs text-slate-500">盈利系数适用于全部模型，分辨率系数适用于图片与视频</p></div><span className="whitespace-nowrap rounded-md bg-white px-2 py-1 text-[11px] font-medium text-violet-700 shadow-sm">所有模型共享</span></div><div className="grid gap-4 lg:grid-cols-[180px_minmax(0,1fr)]"><div className="rounded-lg border border-violet-100 bg-white p-3"><NumberInput label="全局盈利系数" value={config.profitMultiplier} step={0.1} onChange={(v) => setConfig({ ...config, profitMultiplier: v })} /></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{(query.data?.resolutions ?? []).map((resolution) => <div key={resolution} className="rounded-lg border border-violet-100 bg-white p-3"><NumberInput label={`${resolution} 系数`} value={config.resolutionFactors[resolution] ?? 1} step={0.05} onChange={(value) => setConfig({ ...config, resolutionFactors: { ...config.resolutionFactors, [resolution]: value } })} /></div>)}</div></div></div>
      </div>
...
function CatalogModelPriceRow({ model, canEdit, globalProfitMultiplier }: { model: PricingData['models'][number]; canEdit: boolean; globalProfitMultiplier: number }) {
  ...
  return <tr className="align-middle hover:bg-slate-50/70">
    ...
    <td className="px-5 py-4 text-right">{canEdit && <button type="button" disabled={save.isPending} onClick={() => save.mutate()} className="inline-flex items-center gap-1 rounded-lg border border-violet-200 bg-violet-50 px-3 py-2 text-xs font-medium text-violet-700 hover:bg-violet-100 disabled:opacity-50"><Save className="h-3.5 w-3.5" />保存</button>}{save.isError && <ErrorText error={save.error} />}</td>
  </tr>;
}
...
export function QueueTab() {
  ...
  return <div className="mx-auto max-w-6xl space-y-4"><div className="flex items-center justify-between"><Info>只有尚未提交渠道的任务可以直接取消并进入退款流程；提交状态未知时禁止直接重试，必须先人工对账。</Info><select value={status} onChange={(e) => setStatus(e.target.value)} className="ml-4 rounded border border-slate-200 bg-white px-3 py-2 text-sm"><option value="">全部状态</option>{['queued','claimed','ready_to_submit','submit_started','submitted','polling','succeeded','failed','cancelled','submission_unknown'].map((s) => <option key={s}>{s}</option>)}</select></div>{query.isLoading ? <Loading /> : <section className="overflow-hidden rounded-xl border border-slate-200 bg-white"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-slate-500"><tr><th className="p-3">一级模型</th><th className="p-3">渠道 / 精确模型</th><th className="p-3">状态</th><th className="p-3">优先级</th><th className="p-3">排队时间</th><th className="p-3 text-right">操作</th></tr></thead><tbody className="divide-y">{query.data?.items.map((row) => <tr key={row.id}><td className="p-3 font-medium">{row.productModelCode}</td><td className="p-3">{row.provider}<div className="text-xs text-slate-500">{row.exactModel}</div></td><td className="p-3">{row.status}</td><td className="p-3">{row.priority}</td><td className="p-3">{new Date(row.queuedAt).toLocaleString()}</td><td className="p-3 text-right">{canManage && row.status === 'queued' && <><button onClick={() => action.mutate({ id: row.id, kind: 'priority', body: { priority: Math.min(100, row.priority + 10), note: '后台人工加急' } })} className="mr-3 text-violet-700">加急</button><button onClick={() => action.mutate({ id: row.id, kind: 'cancel', body: { note: '后台取消未提交任务' } })} className="text-red-600">取消</button></>}{canManage && row.status === 'failed' && !row.submittedAt && <button onClick={() => action.mutate({ id: row.id, kind: 'retry' })} className="text-violet-700"><RefreshCw className="inline h-4 w-4" /> 重试</button>}{canManage && row.status === 'submission_unknown' && <><button onClick={() => action.mutate({ id: row.id, kind: 'reconcile', body: { outcome: 'not_submitted', note: '后台对账确认未提交渠道' } })} className="mr-3 text-emerald-700">确认未提交</button><button onClick={() => action.mutate({ id: row.id, kind: 'reconcile', body: { outcome: 'failed', note: '后台对账确认渠道失败' } })} className="text-amber-700">确认失败</button></>}</td></tr>)}</tbody></table>{!query.data?.items.length && <Empty text="当前没有排队任务" />}</section>}</div>;
}
...
function Input({ value, onChange, placeholder }: { value: string; onChange: (v: string) => void; placeholder?: string }) { return <input value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="rounded border border-slate-200 px-3 py-2 text-sm" />; }
function NumberInput({ label, value, onChange, step = 1 }: { label: string; value: number; onChange: (v: number) => void; step?: number }) { return <label className="text-xs text-slate-500">{label}<input type="number" min={0} step={step} value={value} onChange={(e) => onChange(Number(e.target.value))} className="mt-1 w-full rounded border border-slate-200 px-2 py-2 text-sm" /></label>; }
