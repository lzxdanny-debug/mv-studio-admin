...
export function PaginationBar({
  ...
  const btnActive = 'bg-blue-600 text-white border border-blue-600 shadow-sm';
  const btnDisabled = 'opacity-30 cursor-not-allowed';

  const selectCls =
    'h-7 pl-2 pr-6 text-xs border border-slate-200 rounded-lg bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-400';
          ...
          {totalPages !== undefined && totalPages > 1 && (
            <div className="flex items-center gap-1.5 ml-1 pl-2 border-l border-slate-200">
              <input
                ...
                placeholder="页码"
                className="w-14 h-7 px-2 text-xs text-center border border-slate-200 rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-blue-400"
                aria-label="跳转页码"
              />
              <button
                type="button"
